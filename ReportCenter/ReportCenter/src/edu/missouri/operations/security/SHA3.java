...
public class SHA3 {
	 ...
	 * 
	 * @param x
	 *            The value of which to calculate the binary logarithm
	...
	private static void keccakFRound(long[] A, long rc) {
		...
		if (SHA3.w == 64) {
			...
			SHA3.B[1] = SHA3.rotate64(
					A[15] ^ (dd = SHA3.C[2] ^ SHA3.rotate64(SHA3.C[4], 1)), 28);
			SHA3.B[2] = SHA3.rotate64(
					A[5] ^ (db = SHA3.C[0] ^ SHA3.rotate64(SHA3.C[2], 1)), 1);
			SHA3.B[3] = SHA3.rotate64(
					A[20] ^ (de = SHA3.C[3] ^ SHA3.rotate64(SHA3.C[0], 1)), 27);
			SHA3.B[4] = SHA3.rotate64(
					A[10] ^ (dc = SHA3.C[1] ^ SHA3.rotate64(SHA3.C[3], 1)), 62);
			...
			SHA3.B[1] = SHA3.rotate(
					A[15] ^ (dd = SHA3.C[2] ^ SHA3.rotate(SHA3.C[4], 1)), 28);
			SHA3.B[2] = SHA3.rotate(
					A[5] ^ (db = SHA3.C[0] ^ SHA3.rotate(SHA3.C[2], 1)), 1);
			SHA3.B[3] = SHA3.rotate(
					A[20] ^ (de = SHA3.C[3] ^ SHA3.rotate(SHA3.C[0], 1)), 27);
			SHA3.B[4] = SHA3.rotate(
					A[10] ^ (dc = SHA3.C[1] ^ SHA3.rotate(SHA3.C[3], 1)), 62);
	 ...
	 *            The message
	 * @param msgoff
	 *            The number of times to loop has run times the bitrate
	 * @param rr
	 *            Bitrate in bytes
	 * @param ww
	 *            Word size in bytes
	 * @param off
	 *            The offset in the message
	 * @return Lane
	 */
	private static long toLane(byte[] message, int msgoff, int rr, int ww,
			int off) {
		long rc = 0;
	...
	private static byte[] pad10star1(byte[] msg, int len, int r) {
		...
		byte b = (byte) (nbrf == 0 ? 1
				: ((msg[nrf] >> (8 - nbrf)) | (1 << nbrf)));
	 ...
	 *            The partial message
	 * @param msglen
	 *            The length of the partial message
	 */
	public static void update(byte[] msg, int msglen) {
		...
		if (SHA3.mptr + msglen > SHA3.M.length)
			System.arraycopy(SHA3.M, 0,
					SHA3.M = new byte[(SHA3.M.length + msglen) << 1], 0,
					SHA3.mptr);
	 ...
	 * 
	 * @param withReturn
	 *            Whether to return the hash instead of just do a quick squeeze
	 *            phrase and return {@code null}
	 * @return The hash sum, or {@code null} if <tt>withReturn</tt> is
	 *         {@code false}
	 */
	...
	public static byte[] digest(byte[] msg, int msglen, boolean withReturn) {
		...
		if ((msg == null) || (msglen == 0))
			...
			if (SHA3.mptr + msglen > SHA3.M.length)
				System.arraycopy(SHA3.M, 0, SHA3.M = new byte[SHA3.M.length
						+ msglen], 0, SHA3.mptr);
			System.arraycopy(msg, 0, SHA3.M, SHA3.mptr, msglen);
