...
public class Reports extends OracleQuery {

	static final transient Logger logger = LoggerFactory.getLogger(Reports.class);

	public Reports() {
		super(Pools.getConnectionPool(Pools.Names.REPORTCENTER));
		setQueryString("select * from reports");
		setRowQueryString("select * from reports where id = ?");
	...
	public int storeRow(Connection conn, Item row) throws UnsupportedOperationException, SQLException {
		...
		int retval = 0;
		try (CallableStatement call = conn.prepareCall("{ ? = call reports.report(?,?,?,?,?,"
				+ "?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?) }")) {
