...
public class ManualItemWorkflowEnabledEditorView extends StandardEditorView implements WorkflowEnabled {
	...
	protected Button approvedAsNotedButton;
	protected Button approvedResubmitButton;
	...
	private boolean approvedAsNotedAllowed = true;
	private boolean approvedResubmitAllowed = true;
	
	private boolean previousAllowed = false;
	...
	public ManualItemWorkflowEnabledEditorView() {
	}
	...
	public void attach() {
		...
		disapproveButton = new Button() {
			{
				...
				addClickListener(new Button.ClickListener() {
					...
					public void buttonClick(ClickEvent event) {
						...
						if (approvalComments.getValue() == null || "".equals(approvalComments.getValue())) {
		...
		approvedAsNotedButton = new Button() {
			{
				setCaption(st.approvedAsNotedButton());
				setDescription(st.approvedAsNotedButton_help());
				setImmediate(true);
				setStyleName("workflowbutton");
				setIcon(new ThemeResource("icons/workflow/Cyan_Normal_24x24.png"));
				...
				addClickListener(new Button.ClickListener() {
					...
					public void buttonClick(ClickEvent event) {
						...
						logger.debug("Workflow Approved As Noted Clicked");
						if (approvalComments.getValue() == null || "".equals(approvalComments.getValue())) {
							Notification.show(st.comments_error());
						} else {
							...
							workflowManipulator.workflowApproveAsNoted();
							resetScreen();
						}
		...
		approvedResubmitButton = new Button() {
			{
				setCaption(st.approvedResubmitButton());
				setDescription(st.approvedResubmitButton_help());
				setImmediate(true);
				setStyleName("workflowbutton");
				setIcon(new ThemeResource("icons/workflow/Cyan_Normal_24x24.png"));
				...
				addClickListener(new Button.ClickListener() {
					...
					public void buttonClick(ClickEvent event) {
						...
						logger.debug("Workflow Approved Resubmit Clicked");
						if (approvalComments.getValue() == null || "".equals(approvalComments.getValue())) {
							Notification.show(st.comments_error());
						} else {
							...
							workflowManipulator.workflowApproveResubmit();;
							resetScreen();
						}
		...
		previousButton = new Button() {
			{
				...
				addClickListener(new Button.ClickListener() {
					...
					final boolean notimplemented = true;

					@Override
					public void buttonClick(Button.ClickEvent event) {

						Notification.show("Send back to previous approvers not implemented yet");

						if (notimplemented) {
							return;
						}

						logger.debug("Workflow Previous Clicked");
						if (approvalComments.getValue() == null || "".equals(approvalComments.getValue())) {
							Notification.show(st.comments_error());
						} else {
							...
							// TODO this should take a previous workflow approvalid.
							String approvalId = "";
							workflowManipulator.workflowPrevious(approvalId);
							resetScreen();
				...
				addComponent(new HorizontalLayout() {
					{
						...
						addComponent(approvedAsNotedButton);
						addComponent(approvedResubmitButton);
	...
	public void setWorkflowComponentState(WorkflowComponentState state) {
		...
		case APPROVALNEEDED:
			...
			if (workflowAllowed) {
				...
				approvedAsNotedButton.setVisible(approvedAsNotedAllowed);
				approvedAsNotedButton.setEnabled(approvedAsNotedAllowed);
				
				approvedResubmitButton.setVisible(approvedResubmitAllowed);
				approvedResubmitButton.setEnabled(approvedResubmitAllowed);
				...
				approvedAsNotedButton.setVisible(false);
				approvedAsNotedButton.setEnabled(false);
				
				approvedResubmitButton.setVisible(false);
				approvedResubmitButton.setEnabled(false);
		...
		case INWORKFLOW:
			...
			if (User.canDo(User.getUser().getUserId(), "WORKFLOWADMIN", null, "WORKFLOWADMIN.OVERRIDE") 
					|| SecurityGroupUsers.memberOf("ADMINISTRATORS", User.getUser().getUserId())
				...
				previousButton.setVisible(previousAllowed);
				previousButton.setEnabled(previousAllowed);
	...
	public void setSkipAllowed(boolean skipAllowed) {
		this.approvedAsNotedAllowed = skipAllowed;
	}
	...
	public void setPreviousAllowed(boolean previousAllowed) {
