ALTER TABLE APPLICATIONS INMEMORY;
ALTER TABLE APPLICATIONRIGHTS INMEMORY;
ALTER TABLE SECURITYGROUPS INMEMORY;
ALTER TABLE ROLES INMEMORY;
ALTER TABLE USERS INMEMORY;

CREATE OR REPLACE VIEW PERMISSIONSVIEW AS
SELECT 
	B.ID,
...
INNER JOIN APPLICATIONS C ON
	C.ID = A.APPLICATIONID
INNER JOIN PERMISSIONS B ON
	B.APPLICATIONPERMISSIONID = A.ID 
INNER JOIN APPLICATIONRIGHTS D ON
	D.ID = B.APPLICATIONRIGHTID AND
